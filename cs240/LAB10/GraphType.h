...
void Floyds(){
	allPairsShortestPaths(edges, numVertices);
}

void allPairsShortestPaths(VertexType* edg[50], int N){
	//Triple for loop for the implementation of Floyd's Algorithm
	//Complete this code.
      ...
      while (!vertexQ.IsEmpty())
      {
        ...
        if (!graph.IsMarked(vertex))
        {
          ...
          item.distance = minDistance +
              std::atoi(graph.WeightIs(item.fromVertex, vertex).c_str());
          pq.Enqueue(item);
        }
      }
    }
...
template<class VertexType>
string GraphType<VertexType>::WeightIs
